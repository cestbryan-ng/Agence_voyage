...
const App1 = ({navigation}) => {
  const connexion = async () => {
    ...
    if (username == '') {
      Alert.alert('Erreur de saisie', "Entrer votre nom d'utilisateur.");
      return;
    }
    if (password == '') {
      Alert.alert('Erreur de saisie', 'Entrer votre mot de passe.');
      return;
    }
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.container3}>
        ...
        />
        <TouchableOpacity
          style={{marginTop: -30, alignSelf: 'flex-end', marginRight: 10}}
          onPress={voirpassword}>
          <Image source={require('./images/vue.png')} />
        </TouchableOpacity>
        <TouchableOpacity
          style={{alignSelf: 'flex-end', marginTop: 40}}
          onPress={passwordoublie}>
          <Text
            style={{
              color: '#3B82F6',
              paddingBottom: 5,
        ...
        <Text style={{alignSelf: 'center', paddingTop: 15, paddingBottom: 15}}>
          ou
        </Text>
        <TouchableOpacity
          style={{
